...
class Searcher(indexPath : String)  {
  ...
  def tryPath(path: String, needle: String) : Iterable[SearchResult] = {
    var mine = myTryPath(path, needle).toVector;
    // var theirs = theirTryPath(path, needle).toVector;

    // (mine filterNot (theirs.contains)).foreach(
    //   e => println("mine has " + e + " but theirs doesn't")
    // )

    // (theirs filterNot (mine.contains)).foreach(
    //   e => println("theirs has " + e + " but mine doesn't")
    // )

    return mine;
  }

  def myTryPath(path: String, needle: String) : Iterable[SearchResult] = {
    val reverseIndexForPath = index.fileToWordToLines(path);

    val lines = reverseIndexForPath.filter { case (k, v) => k.contains(needle) }.map { case(w, l) => l }.flatten.to[Set]

    if (lines.size > 0) {
      return lines.map(l => new Match(path, l))
    }

    return Nil
  }

  def theirTryPath(path: String, needle: String) : Iterable[SearchResult] = {
    try {
